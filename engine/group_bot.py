...
class group(vk):
	def __init__(self, token, id=174145768, testing_mode=False, min_wait_time=3, max_wait_time=6,logger_name='vk', log_level='info', api_version=5.9, group_admin=None):
		...
		if self.group_admin != None: self.group_admins = [[i.get('id'), i.get('role')] for i in group_admin.get_group_members(self.id, filter='managers')]
	...
	def set_group_admins(self, admin_class):
		self.group_admins = [[i.get('id'), i.get('role')] for i in admin_class.get_group_members(self.id, filter='managers')]
	...
	def msg_spammer(self, msg, ids=None):
		...
		if ids==None: ids=self.get_group_members(self.id)
		id_list = ''
