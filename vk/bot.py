...
class bot(vk):
	def __init__(self, token, testing_mode=False,logger_name='bot', log_level='info',min_wait_time=3, max_wait_time=6,api_version=5.90):
	...
	def msg_spammer(self, msg, count, ids=None):#working
		...
		while sent < len(ids):
			...
			if r[sent] is list:
				...
				self.log.debug('sent') # TODO: make it better lol
				sent+=1
	...
	def comment_spammer(self, group_ids, messages, posts_per_group=5, attachment='', offset = 1):#working
		...
		for i in group_ids:#each group
			...
			self.log.debug(last_posts + ' - last posts')
			...
			for j in range(0, len(last_posts)):#each post
				...
				r=self.jsoner(self.comment(int(i), int(last_posts[j].get('id')), messages, attachment))#request
				if r.get('error_code') != None:
					self.log.error('comment didn\'t gone, reason(if its num contact author): '+str(r.get('error_code')))
					if r.get('error_code') == 'captcha': self.enter_captcha()
	...
	def add_friends(self, count, ids=None, msg = 'Привет, где-то тебя видел. На всякий случай добавь в друзья.)'):
		...
		while friends_added < len(ids):
			r = self.add_friend(ids[i], msg)#TODO: use offset
			i+=1
			...
			t = randint(self.min_wait_time, self.max_wait_time) # time to wait before next request
			self.log.info('sleeping for ' + str(t))
			sleep(t)
