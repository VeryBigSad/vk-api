...
class vk:
    def __init__(self, token, testing_mode=False, min_wait_time=3, max_wait_time=6, logger_name='vk', log_level='info',
                 api_version=str(5.92)):  # 20.12 - работает
        ...
        self.stub = {'error': {'error_code': 'stub', 'request_params': [{'key': 'method', 'value': 'stub.stub'},
                                                                        {'key': 'v', 'value': '5.92'}]}}
    ...
    def get_group_members(self, group_id, sort='id_asc'):  # 20.12 - работает
        ...
        try:
            ...
            self.log.error('ERROR! ' + self.get_group_members.__name__ + ', resp:\n' + r);return r
        except LookupError:
            self.log.error('ERROR! ' + self.get_group_members.__name__ + ', resp:\n' + r);return r
    ...
    def get_usrinfo(self, url, fields=None):  # 20.12 - работает
        ...
        # except TypeError:
            ...
            # else:
            #     url = url[url.find('https://vk.com/') + 15:len(
            #         url)]  # deleting everything except screen name (like vk.com/durov->durov)
    ...
    def get_user_real_name(self, user_id):
        req_user = self.get_usrinfo(user_id)
        return req_user['first_name'] + ' ' + req_user['last_name']

    def last_seen_or_online(self, user_ids):
        r = self.get_usrinfo(user_ids, fields='online,last_seen')
        self.log.debug(r)
        if type(r) != list:
            r = [r]
        result = []
        for i in r:
            if i.get('online') == 1:
                if i.get('online_mobile') == 1:
                    result.append({'id': i.get('id'), 'online': True, 'mobile': True})
                else:
                    result.append({'id': i.get('id'), 'online': True, 'mobile': False})
            else:
                result.append({'id': i.get('id'), 'online': False, 'mobile': False})
        return result

    def post(self, owner_id, msg='ыыы', attachments='', time='', from_group='', is_ad=''):  # 20.12 - работает
        # публикует запись на стене
        params = {'owner_id': owner_id, 'attachments': attachments, 'message': msg, 'publish_date': time,
                  'from_group': from_group, 'guid': randint(0, 100000), 'marked_as_ads': is_ad}
    ...
    def get_msg_by_id(self, ids, fields=''):
        return self.jsoner(self.method('messages.getById', {'message_ids': ids, 'fields': fields}))

    def msg(self, msg, user_id):  # 20.12 - работает
        # отправляет сообщение человеку
        params = {'message': msg, 'user_id': user_id}
    ...
    def comment(self, owner_id, post_id, msg, attachments='', from_group=''):  # 20.12 - работает
        ...
        params = {'owner_id': owner_id, 'post_id': post_id, 'attachments': attachments, 'message': msg,
    ...
    def get_rand_ids(self, count, last_seen_days_max=3, start_id=1):  # 20.12 - работает
        ...
        params = {'user_id': start_id, 'count': real_count, 'offset': offset,
        ...
        while True:
            ...
            if usr.get('last_seen').get('time') + 259200 < self.start_time:
                ...
                removed += 1
                self.log.debug(
                    str(usr.get('id')) + ' has been removed of he\'s not last_seen recently, totaly: ' + str(removed))
                continue
